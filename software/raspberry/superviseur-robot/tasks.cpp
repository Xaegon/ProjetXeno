...
void Tasks::ReceiveFromMonTask(void *arg) {
    ...
    while (1) {
        ...
        if (msgRcv->CompareID(MESSAGE_MONITOR_LOST)) {
            ...
            cam = cam==0 ? new Camera(sm, 20) : cam;
            cout << "Created camera" << endl << flush;
            // Open the camera
            if (cam->Open()) {
                cout << "Opened camera successfully" << endl << flush;
